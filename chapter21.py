...
def make_env(scenario_name, N=2, max_cycles=60):
    # 从环境文件脚本中创建环境
    # scenario = scenarios.load(scenario_name + ".py").Scenario()
    # world = scenario.make_world()
    # env = MultiAgentEnv(world, scenario.reset_world, scenario.reward,
    #                     scenario.observation)
    env = simple_adversary_v3.parallel_env(N=2, max_cycles=max_cycles, render_mode="rgb_array")
    # env = simple_adversary_v3.env(N=2, max_cycles=max_cycles, render_mode="rgb-array")
    return env
...
env = make_env(env_id, N = 2, max_cycles=episode_length)  ## 产生多智能体的环境
env.reset(seed = 0)
replay_buffer = ReplayBuffer(buffer_size)  ## 历史数据的回放池

state_dims = []
action_dims = []
for _, action_space in env.action_spaces.items():       ## 遍历所有智能体的动作空间，拿到每一个智能体动作的 dim
    action_dims.append(action_space.n)
for _, state_space in env.observation_spaces.items():   ## 遍历所有智能体的状态空间，拿到每一个智能体状态的 dim
...
def evaluate(env_id, maddpg, n_episode=10, episode_length=60):
    # 对学习的策略进行评估,此时不会进行探索
    env = make_env(env_id, N = 2, max_cycles=episode_length)
    env.reset(seed = 100)
    ...
    for _ in range(n_episode):
        ...
        for t_i in range(episode_length):
            ...
            actions = {env.agents[agent]: np.argmax(actions[agent]) for agent in range(len(env.agents))}
            next_state, reward, terminations, truncations, infos = env.step(actions)
            obs, rew, done = convert(next_state, reward, terminations, truncations)
            rew = np.array(rew)
...
def getstate(env):
    ...
    for agent in env.agent_iter():
        ...
        r.append(reward)
        done.append(termination | truncation)
        num += 1
        if num==3:
            return state, r, done
...
def convert(next_state, reward, terminations, truncations):
    if len(env.agents)==0:
        agents = env.possible_agents
    else:
        agents = env.agents
    next_state = [next_state[agent] for agent in agents]
    reward = [reward[agent] for agent in agents]
    terminations = np.array([terminations[agent] for agent in agents])
    truncations = np.array([truncations[agent] for agent in agents])
    return next_state, reward, terminations | truncations
...
for i_episode in range(num_episodes):
    state, infos = env.reset()
    state = [state[agent] for agent in env.agents]
    # state,_,_ = getstate(env)
    ...
    for e_i in range(episode_length):
        ...
        act = {env.agents[agent]: np.argmax(actions[agent]) for agent in range(len(env.agents))}
        next_state, reward, terminations, truncations, infos = env.step(act)
        next_state, reward, finish = convert(next_state, reward, terminations, truncations)
        # next_state, reward, done, _ = env.step(actions)
        # next_state, reward, done = envstep(env, actions)
        done = finish
        # state,_,_ = getstate(env)
...
imageio.mimsave(os.path.join(pth, 'chapter%s.gif'%str(21)), allimage, duration=60)
env.close()
