...
class TRPO:
    ...
    def hessian_matrix_vector_product(self, states, old_action_dists, vector):
        ...
        kl = torch.sum(out)
        # new_action_dists = torch.distributions.Categorical(out)    ## 策略网络给出选择动作的概率，然后依概率采样
        ...
        kk = torch.sum(kl_grad_vector)
        gradkk = torch.autograd.grad(kk, out, retain_graph=True)
        # KL距离的梯度先和向量进行点积运算
        ...
        '''
        grad2 = torch.autograd.grad(kl_grad_vector_product, kl_grad, retain_graph=True)
        grad6 = torch.autograd.grad(kl_grad_vector_product, self.actor.parameters(), retain_graph=True)
...
agent = TRPO(hidden_dim, env.observation_space, env.action_space, lmbda,
             kl_constraint, alpha, critic_lr, gamma, device)
return_list = train_on_policy_agent(env, agent, num_episodes, "CartPole")
